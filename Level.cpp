namespace Tmpl8
{
	Surface tileset("assets/_Tileset.jpg");

	Sprite backdrop[4] = {  // Layered Background for Potential Parallax
	...
	const char map1[16][75] = {  // Tedious Tilemap
				 "je je je ja ja ja ja ja ja ja ja ja ja ja ja ja ja ja ja ja ja ja ja ja ja",
				 ...
				 "aaGaaGaaGjaGjaGja cb je je ab db bb bb bb db bb dc bb bb bb dc caHja ja aa",
				 "jaGjaGjaGjaGjaGja eb da da ga bb bb dc bb bb bb bb db bb bb bb eb caBjaCab",
	};

	// Level Class for drawing the level onscreen and storing levels
	class Level
	{
	public:
	...
	private:
		const int m_height = 16, m_width = 25;
		...
		for (int y = 0; y < m_height; y++)
			for (int x = 0; x < m_width; x++)
			{
				...
				int flair = (*m_map)[y][x * 3 + 2]; // foreground and background decorative elements
												   // assigned per-tile
				Pixel* src = tileset.GetBuffer() + tx * 32 + (ty * 32) * 320;
				Pixel* dst = screen->GetBuffer() + x * 32 + y * 32 * 800;

				// Decorarive Background elements
				if (flair >= 'A' && flair <= 'O')
				{
					...
					for (int i = 0; i < 32; i++)
					{
						...
						// Group D (ty = 3)
							...
							{
								int fx = flair - 'A';
								src = tileset.GetBuffer() + fx * 32 + (3 * 32) * 320 + (i * 320);
								if (src[j] >= 0x88000000)
									dst[j] = src[j];
							}
							else if (flair == 'B')
							...
							{
								int fx = flair - 'F';
								src = tileset.GetBuffer() + fx * 32 + (4 * 32) * 320 + (i * 320);
								if (src[j] >= 0x88000000)
									dst[j] = src[j];
							}
							else if (flair == 'G')
							...
							{
								int fx = flair - 'K';
								src = tileset.GetBuffer() + fx * 32 + (5 * 32) * 320 + (i * 320);
								if (src[j] >= 0x88000000)
									dst[j] = src[j];
							}
							else if (flair == 'L')
				...
				// Drawing the Main Tile 
					dst = screen->GetBuffer() + x * 32 + y * 32 * 800;
					src = tileset.GetBuffer() + tx * 32 + (ty * 32) * 320;
					for (int i = 0; i < 32; i++)
					{
						for (int j = 0; j < 32; j++)
							if (src[j] >= 0x88000000)
				...
				if (flair >= 'g' && flair <= 'j')
				{
					...
					for (int i = 0; i < 32; i++)
					{
						for (int j = 0; j < 32; j++)
						{
							...
							{
								int fx = flair - 'a';
								src = tileset.GetBuffer() + fx * 32 + (2 * 32) * 320 + (i * 320);
								if (src[j] >= 0x88000000)
									dst[j] = src[j];
							}
							else if (flair == 'h')
	...
	void Level::InitTiles()
	{
		for (int y = 0; y < m_height; y++)
			...
			{
				int tx = (*m_map)[y][x * 3];  
				int ty = (*m_map)[y][x * 3 + 1];

				Tile joe(x, y, tx, ty);
				level_tiles.push_back(joe);
			}
